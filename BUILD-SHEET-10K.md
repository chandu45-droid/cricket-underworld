...
|---|---|---|---|---|---|
| L1 | **Global `.money` hero class** — headline currency solid gold, bigger, ₹ symbol, proper number font; route every currency value through it. Use `var(--gold-bright)` (works both themes). | ui-designer | Every ₹ value on hub/auction/match-result renders via `.money`; screenshot diff shows one dominant gold number per screen; tests green | S | First thing a buyer notices; "money looks like money" = perceived value |
| L2 | **Gold austerity pass** — strip gold off the ~40 low-opacity panels; reserve gold for ONE hero + primary CTA per screen. | ui-designer | Audit list of panels de-golded; each core screen has exactly one gold hero; before/after shots attached | S–M | Rich is a contrast phenomenon; makes the whole game look designed, not templated |
| L3 | **Load Space Grotesk (+ Cinzel if used)** — specced, never loaded; money/stats currently fall back to Teko. | ui-designer | Fonts load (network tab confirms); money/stat text renders in the specced face on real device | S | Typography is 50% of "premium feel" for near-zero effort |
| L4 | **Juice pass** — card-reveal, win/loss celebration moment, screen transitions, SFX/haptic on key taps. | player-experience | 4 named moments animate; 60fps on a budget Android; toast/SFX fire; tests green | S–M | Perceived value per rupee is highest here; makes the demo feel alive |
| L5 | **First-60-seconds reel polish** — splash → onboarding → first match win path is flawless. | player-experience | Fresh-install run recorded; zero jank, zero dead taps, ends on a win celebration | S | The sale is won in the first minute; this IS the trailer |
...
|---|---|---|---|---|---|
| P0 | **Fix demo blocker: tutorial-overlay pointer-block** (HIGH bug — `#tut-overlay.show` intercepts all taps on fresh load). | in-session build | E2E: fresh load → tap nav → screen switches; manual fresh-install confirms | S | A buyer's first click must work or the deal dies in 5 seconds |
