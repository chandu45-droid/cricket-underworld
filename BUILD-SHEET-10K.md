...
| P1 | **Verify live deployment + real-device QA** — confirm `chandu45-droid.github.io/.../prototype/` serves the CURRENT build; check theme + toggle on a real budget Android. *2026-07-15: founder device screenshots confirmed live light theme + caught/fixed splash-logo overflow; full pass still owed after Pillar-3 merge.* | founder-assisted | Public URL shows current build; screenshots from a real device attached | S | The link IS the pitch; it must be the polished build, not a stale one |
| P2 🔧 | **Distribution** — India-first per §11. **KIT READY 2026-07-15** (`distribution/DISTRIBUTION-KIT.md`): channel/wave plan (r/indiangaming, r/WebGames, X, WhatsApp/Telegram, r/cricket last), ready-to-paste posts, per-wave UTM tags (game now captures first-touch UTM/referrer → attributable cohorts), gate metrics to watch. Founder fires wave 1 after deploying the collector. | sanjaya research done in kit + founder | ≥3 seed posts live; UTM/referrer visible in analytics *(capture side DONE + E2E)* | S (founder-assisted) | Turns "zero users" into a retention curve you can price |
| P3 🔧 | **Collect the signal** — **PIPELINE BUILT 2026-07-15**: remote sink beacon in-game (`flushAnalytics`, boot/debounce/pagehide, cursor + retry, offline-safe, endpoint-pluggable) + free Google Apps Script collector (`analytics/apps-script-sink.gs` → Sheet; GET = fleet D1/D7 JSON) + deploy guide (`analytics/README.md`). **Founder action (~3 min): deploy script, set `ANALYTICS_ENDPOINT`, push.** Then freeze until ~200 users / 2 weeks. | Chanakya (tracking) | ≥200 users OR 2 weeks of data; D7 computed *(computable the moment the endpoint is live)* | — | The number that reprices the whole asset |
| P4 ✅ | **DONE 2026-07-15** — 43s reel recorded from REAL gameplay at 390×844 light theme (`docs/demo/cricket-underworld-60s-reel.webm` + README with beats/regen/convert notes): splash → hub → 6-step tutorial → live auction (3 bids) → XI confirm → live ball-by-ball sim → deterministic **Victory** end-beat (god-squad vs seeded trash XI, the L5 technique). Frame-verified: no white lead, no toast overlap, logo fits. | player-experience + founder | ✅ Video renders the first-60s reel + a win; <60s | S | The artifact that travels; sits atop `pitch.html` |
| P5 | **Wire live metrics into `pitch.html`** once ~2 weeks of data exist; refresh `outreach.md` with real numbers. *(Collector's GET endpoint already returns the JSON to wire in.)* | in-session build | pitch.html shows live D1/D7 + install count; outreach.md numbers match reality | S | Data-backed pitch = the difference between $3k and $10k |
