...
**Resolved 2026-09-09 (founder call, both CUT from GDD, not built):** RTM (Right to Match) assumed a season-boundary squad wipe/re-auction that the shipped game never has (`endSeason()` never resets `GS.squad`) -- the dead `#rtm-banner` CSS/markup stub was removed from `prototype/index.html`. "Planted Agent" mole mechanic assumed a rival-AI "strategy" data model that doesn't exist anywhere in the code (`GS.rivalData` is relationship-bookkeeping only) -- never built, formally cut. Also caught while in the same GDD table: `Auction Leak` was already cut from code back on 2026-08-03 (bug-audit pass) but the GDD table was never updated until now. See `docs/core-systems-gdd.md` §5.2/§5.4/§8.4/§9.2 for the corrected text.
